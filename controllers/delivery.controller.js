...
exports.acceptDeliveryTask = async (req, res) => {
  try {
    console.log("=== ACCEPT DELIVERY TASK API CALLED ===");
    ...
    if (delivery.isAccepted) {
      return res.status(400).json({
        success: false,
        message: "Delivery task is already accepted",
      });
    }

    // Accept the assignment
    delivery.deliveryPerson = req.user.id; // Ensure delivery person is set
    delivery.status = "picked_up"; // Move to next status after acceptance
...
exports.rejectDeliveryTask = async (req, res) => {
  try {
    console.log("=== REJECT DELIVERY TASK API CALLED ===");
    console.log("Task ID:", req.params.id);
    console.log("User ID:", req.user?.id);
    ...
    console.log("Updating delivery status to rejected...");
    ...
    if (delivery.order) {
      try {
        ...
        if (order) {
          ...
          order.orderStatus = "confirmed"; // Back to confirmed for reassignment
          await order.save();
          console.log("Order updated successfully");
        }
      } catch (orderError) {
        console.log("Order update failed, but continuing:", orderError.message);
      }
    }
    ...
    console.error("=== REJECT DELIVERY TASK ERROR ===");
...
// @desc    Get all delivered orders for table display
// @route   GET /api/delivery/all-delivered-orders
// @access  Private
exports.getAllDeliveredOrders = async (req, res) => {
  try {
    console.log("=== GET ALL DELIVERED ORDERS ===");

    // Find all deliveries with status 'delivered' - simplified query
...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    // 5. Rejected Orders Count
    const rejectedOrders = await Delivery.countDocuments({
      ...baseQuery,
      status: "rejected",
    });
